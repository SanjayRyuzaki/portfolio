                ...
                <div className="mt-8">
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    {['Creative', 'Hardworking', 'Adaptive', 'Curious', 'Collaborative'].map((trait) => (
                      <span
                        key={trait}
                        className="px-3 py-1 md:px-4 md:py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 rounded-full text-xs md:text-sm font-medium"
                      >
                        {trait}
                      </span>
                    ))}
              ...
              <div className="space-y-4 md:space-y-6">
                {experiences.map((exp, index) => (
                  <motion.div
                    ...
                    className="bg-white/50 backdrop-blur-sm p-4 md:p-6 rounded-2xl border border-gray-200 dark:bg-gray-800/50 dark:border-gray-700 hover:shadow-lg transition-all duration-300"
                  >
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                      <div className="flex-1">
                        <h4 className="text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2">
                          {exp.title}
                        </h4>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {exp.skills.map((skill) => (
                        <span
                          key={skill}
                          className="px-2 py-1 md:px-3 md:py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 rounded-full text-xs font-medium"
                        >
                          {skill}
                        </span>
                      ))}
